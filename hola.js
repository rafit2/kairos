...
function BMI(){
    ...
    if (BMI<=18.5) {                                    // Need to handle multiple cases
        ...
        var status = "obeso extremo"
    } 
                 // Calculates the IMC using height and weight
    
    const node = document.createTextNode("Su IMC es igual a  "+ strBMI +", que dice que usted está " + status); // Create a child node where text will be stored 
